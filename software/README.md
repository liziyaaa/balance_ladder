...
| 采样周期 | 控制环 4 ms，即 250 Hz |
| 互补滤波系数 | `alpha = 0.98` |
| MPU6050 DLPF | `DLPF_CFG=1`，降低姿态滞后 |
| 陀螺仪量程 | ±500 deg/s |
| 加速度计量程 | ±2 g |
| 控制角速度 | `gyro_y` |
...
当前控制周期为 250 Hz，默认参数在 `ControlParams` 中：
...
| `gain_pos` | `1.00` | 正向风力补偿增益 |
| `gain_neg` | `1.35` | 反向风力补偿增益 |
| `min_pos` | `0.18` | 正向小输出死区补偿 |
| `min_neg` | `0.24` | 反向小输出死区补偿 |
| `kick_pos` | `0.24` | 正向起转/反向切换短促补偿 |
| `kick_neg` | `0.34` | 反向起转/反向切换短促补偿 |
| `baseline_cmd` | `0.0` | 基准电机命令补偿 |

参数会保存到 NVS，复位后会继续使用上次通过 BLE 设置的值。如果需要恢复当前代码里的默认值，可以通过 BLE 发送 `defaults`。
...
actual_cmd = motor_set(final_cmd);
```
...
2. 只调 `kp`，看它是否有明确回正趋势。
3. 加 `kd` 抑制快速振荡和过冲。
4. 如果小误差附近电机不动，先调对应方向的 `min_pos/min_neg`；如果反向切换慢，再调对应方向的 `kick_pos/kick_neg`。
5. 不要急着大幅增加 `kp`，否则会变成过冲和来回摆。
6. 若存在固定偏置，再小幅调整 `baseline` 或最后加入很小的 `ki`。
...
| 正反向增益补偿 | `cmd > 0` 使用 `gain_pos`，`cmd < 0` 使用 `gain_neg` |
| 小输出补偿 | 正向使用 `min_pos`，反向使用 `min_neg` |
| 起转/反向补偿 | 命令方向变化且命令不太小时，40 ms 内至少输出对应方向 `kick_pos/kick_neg` |
| 最终限幅 | 补偿后仍受 `output_limit` 约束 |
...
正桨在正反转时推力不对称，因此固件把不对称补偿放在电机输出层：PID 先计算统一的控制命令，`motor_set()` 再按命令正负分别套用 `gain/min/kick`。如果某一边风力弱，优先调弱风方向的 `gain_*`、`min_*` 和 `kick_*`，不要先把 PID 分成两套。
...
`gyro` 当前是 `gyro_y`，单位 deg/s。BLE 任务每 50 ms 检查一次遥测，`ble_debug.cpp` 对 Notify 做了 100 ms 限速，因此当前实际最大发送频率约为 10 Hz。控制环本身是 250 Hz。
...
|---|---|
| `status` | 立即回传状态和参数 |
| `defaults` | 恢复默认 PID、目标角、输出限幅、正反向电机补偿参数和 `baseline` |
...
| `gain_pos=1.00` | 设置正向风力补偿增益 |
| `gain_neg=1.35` | 设置反向风力补偿增益 |
| `gain=1.00` | 同时设置正反向补偿增益 |
| `min_pos=0.18` | 设置正向最小有效命令补偿 |
| `min_neg=0.24` | 设置反向最小有效命令补偿 |
| `min_cmd=0.18` | 兼容命令，同时设置 `min_pos/min_neg` |
| `kick_pos=0.24` | 设置正向起转/切换补偿，`0` 表示关闭 |
| `kick_neg=0.34` | 设置反向起转/切换补偿，`0` 表示关闭 |
| `kick=0.24` | 兼容命令，同时设置 `kick_pos/kick_neg` |
| `baseline=0` | 设置基准输出补偿 |
...
OLED 绝不能放进 250 Hz 控制环里刷新；当前实现已经放在低优先级 `ui_task`。
...
| 任务 | 周期/触发 | 优先级 | 说明 |
|---|---|---:|---|
| `control_task` | 250 Hz | 10 | 读取 IMU、判断安全、更新 PID、输出电机 |
...
5. 连接 BLE，先发 `status`，确认目标角、PID、正反向电机补偿参数和 `baseline` 与预期一致。
6. 在 `DISARMED` 下用 `motor=0.2`、`motor=0.5`、`motor=1` 确认推力方向和 PWM 响应。
7. 装桨后手持平衡梯，观察倾倒时 `cmd` 的方向是否能抵消倾倒。
8. 扶正后短按 GPIO1 进入 `ARMED`，先只调 `kp/kd/gain_pos/gain_neg/min_pos/min_neg/kick_pos/kick_neg`。
...
- 如果电机小占空比抖动不转，先调对应方向 `min_pos/min_neg` 或手动 `motor=...` 找最小可转命令；如果某方向反应太慢，再调对应方向 `gain_*` 和 `kick_*`。
- 如果 OLED 或 GPIO9 导致启动异常，可以先断开 OLED 或检查上拉。
- 如果 BLE 遥测频率不够，可以降低 `ble_debug.cpp` 中的 100 ms 限速，但不要影响 250 Hz 控制任务。
