...
|---|---:|---|---|
| 电池电压检测 | GPIO0 | `BAT_ADC` | ADC 输入，R3/R5 分压；当前固件暂未读取 |
...
| 板载状态灯 | GPIO8 | SuperMini 板载 LED | 低电平点亮，用于固件状态提示 |
...
单节锂电满电 `4.2 V` 时，`Vadc` 约为 `3.16 V`，低于 3.3 V ADC 范围。当前固件为了简化闭环调试暂未启用电池检测和低电压停机；若后续重新加入，需要做 ADC 校准、多次平均和低电压阈值判断。
...
当前固件已根据实物电机方向做过一次软件反向：`cmd > 0` 时输出到 `IN2`，`cmd < 0` 时输出到 `IN1`。复刻者如果更换电机线序、桨叶方向或驱动封装，应重新确认推力方向，而不是直接假设表格中的“正向/反向”与自己的实物完全一致。
...
- `GPIO8` 是 SuperMini 板载蓝色 LED，当前固件用于状态提示，默认低电平点亮。
